...
public class PicassoLoader implements ILoaderStrategy {
    ...
    private static Picasso getPicasso() {
        if (sPicassoSingleton == null) {
            synchronized (PicassoLoader.class) {
                if (sPicassoSingleton == null) {
                    sLruCache = new LruCache(application);
                    sPicassoSingleton = new Picasso.Builder(application).memoryCache(sLruCache).build();
                }
            }
        }
    ...
    public void clearDiskCache() {
        File diskFile = new File(application.getCacheDir(), PICASSO_CACHE);
        if (diskFile.exists()) {
            //这边自行写删除代码
